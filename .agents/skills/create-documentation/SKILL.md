...
You are creating proper markdown documentation for a feature or guide in the Sign Leuna documentation site.
...
| ------------------- | ------------------------------------------------ | -------------------------------------------------- |
| **User Guide**      | `apps/docs/content/docs/users/<section>/`        | UI workflows for using the Sign Leuna web app       |
...
description: Upload documents to Sign Leuna to prepare them for signing. Covers supported formats, file size limits, and upload methods.
---
```
...
<Callout type="warn">
  ...
  [OpenAPI Reference](https://openapi.leuna.app).
</Callout>
...
```typescript
const response = await fetch('https://app.leuna.app/api/v2/document', {
  method: 'POST',
  headers: {
...
description: Complete reference for all environment variables used to configure Sign Leuna.
---
