...
## How Sign Leuna Supports Compliance

Sign Leuna provides features that support compliance with e-signature laws across jurisdictions:
...
Sign Leuna maintains an audit trail for each document, recording:
...
## What Sign Leuna Does NOT Provide

Sign Leuna supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
