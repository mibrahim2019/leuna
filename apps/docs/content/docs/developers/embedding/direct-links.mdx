...
When someone uses a direct link, Sign Leuna creates a new document from the template with that person as the signer.
...
curl -X POST "https://app.leuna.app/api/v2/template/direct/create" \
...
// URL: https://app.leuna.app/d/{token}
````
</Tab>
</Tabs>
...
```html
<iframe
  src="https://app.leuna.app/embed/direct/abc123xyz"
  width="100%"
  height="800"
  frameborder="0"
...
function DocumentSigner({ token }: { token: string }) {
  return (
    <div className="h-[800px] w-full">
      <iframe
        src={`https://app.leuna.app/embed/direct/${token}`}
...
function redirectToSigning(token: string) {
  window.location.href = `https://app.leuna.app/d/${token}`;
}
```
...
```
https://app.leuna.app/d/abc123xyz?externalId=order-12345
```
...
function getSigningUrl(token: string, orderId: string) {
  ...
  return `https://app.leuna.app/d/${token}?${params.toString()}`;
}
...
```
https://app.leuna.app/embed/direct/abc123xyz?externalId=order-12345
```
...
| Account   | Signer must be logged into a Sign Leuna account |
...
<Tab value="curl">
```bash
curl -X POST "https://app.leuna.app/api/v2/template/direct/toggle" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123,
...
```bash
curl -X POST "https://app.leuna.app/api/v2/template/direct/delete" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123
...
Enterprise plans support white-label embedding with the "Powered by Sign Leuna" badge removed. [Contact sales](mailto:support@leuna.app) for details.
...
function getEmbedUrl(token: string, orderId: string) {
  ...
  return `https://app.leuna.app/embed/direct/${token}?${params}`;
}
