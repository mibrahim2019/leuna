...
Webhooks are HTTP callbacks triggered by specific events in Sign Leuna. When an event occurs (such as a document being signed), Sign Leuna sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in Sign Leuna, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
<Callout type="warn">
  Always respond with a `200 OK` status within 30 seconds. Sign Leuna will retry failed deliveries.
</Callout>

## Configuring Webhooks in Sign Leuna via the Dashboard
...
Sign Leuna provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
Sign Leuna will attempt to deliver the same payload again
</Step>
</Steps>

## Retry Policy

When a webhook delivery fails (non-2xx response or timeout), Sign Leuna automatically retries with exponential backoff:
...
<Accordions type="multiple">
  ...
  <Accordion title="Verify signatures">
    Use a webhook secret and verify the `X-Sign Leuna-Secret` header (see [Verification](/docs/developers/webhooks/verification)).
  </Accordion>
