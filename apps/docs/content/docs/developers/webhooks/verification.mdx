...
Verifying webhook requests ensures that incoming payloads originate from Sign Leuna and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How Sign Leunas Webhooks

When you configure a webhook with a secret, Sign Leuna includes that secret in every webhook request via the `X-Sign Leuna-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-Sign Leuna-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-Sign Leuna-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-Sign Leuna-Secret` header from the incoming request
  </Step>
  <Step>
...
const WEBHOOK_SECRET = process.env.SIGN_DOCUTRACKER_WEBHOOK_SECRET;
...
WEBHOOK_SECRET = os.environ.get('SIGN_DOCUTRACKER_WEBHOOK_SECRET')
...
def handle_webhook():
    received_secret = request.headers.get('X-Sign Leuna-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in Sign Leuna |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in Sign Leuna             |
...
3. Update the webhook configuration in Sign Leuna
