...
- A Sign Leuna account on any plan (Free, Individual, Team, or Enterprise)
...
- Log in to your Sign Leuna account
- Click your avatar in the top right corner
- Select **Settings** from the dropdown menu

![User dropdown menu](/public-api-images/leuna-user-dropdown-menu.webp)
...
![API tokens page](/public-api-images/api-tokens-page-leuna.webp)
...
![API key display](/public-api-images/leuna-api-key-blurred.webp)
...
```bash
curl https://app.leuna.app/api/v2/documents \
...
```typescript
const response = await fetch('https://app.leuna.app/api/v2/documents', {
  method: 'GET',
  headers: {
...
Sign Leuna provides official SDKs that handle authentication for you:

```typescript
import { Sign Leuna } from '@documenso/sdk-typescript';

const client = new Sign Leuna({
...
<Callout type="info">
  ...
  OpenAPI reference](https://app.leuna.app/api/v1/openapi).
</Callout>
...
SIGN_DOCUTRACKER_API_KEY=api_xxxxxxxxxxxxxxxx
```

```typescript
const client = new Sign Leuna({
  apiKey: process.env.SIGN_DOCUTRACKER_API_KEY,
});
```
