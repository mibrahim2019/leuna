...
description: Create and send a document for signing using the Sign Leuna API, from uploading a PDF to adding recipients and distributing for signature.
---
...
- A Sign Leuna account (cloud or self-hosted)
...
| ----------- | -------------------------------------- |
| Production  | `https://app.leuna.app/api/v2`     |
| Staging     | `https://stg-app.leuna.app/api/v2` |
...
<Tab value="curl">
```bash
curl -X POST "https://app.leuna.app/api/v2/envelope/envelope_abc123/distribute" \
  -H "Authorization: YOUR_API_TOKEN" \
...
const response = await fetch(
  `https://app.leuna.app/api/v2/envelope/${envelopeId}/distribute`,
  {
    method: 'POST',
    headers: {
      Authorization: 'YOUR_API_TOKEN',
...
BASE_URL="https://app.leuna.app/api/v2"
...
- [API Reference](https://openapi.leuna.app/) - Full endpoint documentation with request/response schemas
