...
Register your endpoint in the Sign Leuna dashboard:
...
<Steps>
  ...
  <Step>
    Enter your webhook secret (the same value stored in <code>SIGN_DOCUTRACKER_WEBHOOK_SECRET</code>)
  </Step>
  <Step>
    Click **Create Webhook**
  </Step>
...
class Sign LeunaApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof Sign LeunaApiError) {
...
- [API Reference](https://openapi.leuna.app/) - Full endpoint documentation
