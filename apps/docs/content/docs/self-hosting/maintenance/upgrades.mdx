---
title: Upgrades
description: Upgrade your self-hosted Sign Leuna installation to get the latest features, security patches, and bug fixes.
---
...
Sign Leuna follows [Semantic Versioning](https://semver.org/):
...
Check your running Sign Leuna version:
...
```bash
# In .env
SIGN_DOCUTRACKER_VERSION=1.6.0
```
...
services:
  documenso:
    image: documenso/documenso:${SIGN_DOCUTRACKER_VERSION:-latest}
```
...
Sign Leuna runs database migrations automatically when the container starts. No manual intervention is required.
