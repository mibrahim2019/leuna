---
title: Kubernetes
description: Deploy Sign Leuna on Kubernetes for production environments requiring high availability, auto-scaling, and enterprise-grade infrastructure.
---
...
A typical Sign Leuna Kubernetes deployment consists of:
...
Create a dedicated namespace for Sign Leuna:
...
Create the Sign Leuna Deployment:
...
Update your Sign Leuna secret to use the in-cluster database:
...
Sign Leuna stores documents in the database by default. For high-volume deployments, configure S3-compatible storage.
...
Scale Sign Leuna based on CPU and memory utilization:
