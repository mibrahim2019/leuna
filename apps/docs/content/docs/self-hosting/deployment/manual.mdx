...
description: Deploy Sign Leuna on a Linux server without Docker using Node.js and systemd.
---
...
<Callout type="info">
  If you use a reverse proxy in front of Sign Leuna, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run Sign Leuna as a background service on Linux:

```ini
# /etc/systemd/system/documenso.service

[Unit]
Description=Sign Leuna
...
A minimal Nginx configuration for proxying to Sign Leuna:
