---
title: Docker
description: Run Sign Leuna as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
These variables must be set for Sign Leuna to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where Sign Leuna is accessible (e.g., `https://sign.example.com`) | -                             |
...
Sign Leuna requires two database connection strings:
...
docker run -d \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="Sign Leuna" \
...
Sign Leuna provides health check endpoints for monitoring:
...
To update Sign Leuna to a new version:
...
The Sign Leuna container is stateless. All persistent data is stored in the PostgreSQL database.
