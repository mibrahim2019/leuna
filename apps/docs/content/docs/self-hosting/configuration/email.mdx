...
Sign Leuna supports four email transport methods:
...
SMTP is the most common email transport. Sign Leuna supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for Sign Leuna to use.
...
Sign Leuna uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the Sign Leuna logo
...
Configure Sign Leuna to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="Sign Leuna Test"
```
...
For MailChannels, configure DKIM directly in Sign Leuna using the environment variables described above.
