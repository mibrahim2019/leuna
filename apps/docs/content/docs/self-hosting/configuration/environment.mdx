...
description: Complete reference for all environment variables used to configure a self-hosted Sign Leuna instance.
---

## Required Variables

These variables must be set for Sign Leuna to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your Sign Leuna instance (e.g., `https://sign.example.com`)                   |
...
Sign Leuna requires PostgreSQL 14 or higher.
...
Sign Leuna supports multiple email transports for sending notifications.
...
Sign Leuna can store documents in the database or S3-compatible storage.
...
Sign Leuna requires a certificate to digitally sign documents.
...
Sign Leuna can use Google Vertex AI for recipient and field detection.
...
Sign Leuna supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
...
| Variable                      | Description                                  | Default |
| ----------------------------- | -------------------------------------------- | ------- |
| `SIGN_DOCUTRACKER_DISABLE_TELEMETRY` | Set to `true` to disable anonymous telemetry | `false` |

Telemetry also auto-disables when `NEXT_PRIVATE_SIGN_DOCUTRACKER_LICENSE_KEY` is configured.
