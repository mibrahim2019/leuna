...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your Sign Leuna deployment.
---
...
Sign Leuna requires **PostgreSQL 14 or later**. No other databases are supported.
...
Sign Leuna uses two database connection variables:
...
Sign Leuna automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, Sign Leuna will use them automatically.
...
Sign Leuna uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:
...
Database migrations update your schema when upgrading Sign Leuna.

### Automatic Migrations

When running Sign Leuna via Docker, migrations run automatically on container startup. No manual intervention is required.
...
Key parameters for Sign Leuna workloads:
...
Sign Leuna includes necessary indexes by default. Additional indexes may help for:
