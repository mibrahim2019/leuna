...
S3 storage is recommended for production deployments. Sign Leuna supports AWS S3 and any S3-compatible storage service.
...
Using Docker Compose with Sign Leuna:
...
Sign Leuna works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
- An S3 bucket configured for Sign Leuna
...
Sign Leuna does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, Sign Leuna can read from both backends. The `DocumentData.type` field indicates where each document is stored.
